...
import {
  ...
  Pin, RefreshCw, Star, Plus, Minus, AlertCircle, Loader2, Sun, Moon
} from 'lucide-react';

// ============================================================
// ค่า API KEY อ่านมาจาก .env (VITE_FINNHUB_API_KEY)
// ============================================================
const FINNHUB_API_KEY = import.meta.env.VITE_FINNHUB_API_KEY ?? '';

// ============================================================
// Types & Interfaces — แทน any[]
// ============================================================
interface Stock {
  symbol: string;
  name: string;
...
interface PinnedPrice {
  c: number;
  pc: number;
}

// ============================================================
// ฟังก์ชัน helper ที่ไม่ขึ้นกับ component — อยู่นอก App()
// ============================================================

// ดึงธีมเริ่มต้นจากระบบ
...
const generateChartData = (targetEndPrice: number, points: number, volatility: number): number[] => {
...
// ฟอร์แมตราคา (รองรับ USD / THB)
const formatPrice = (price: number, currency: string, exchangeRate: number): string => {
...
export default function App() {
  // ธีม — ใช้ lazy initializer (ส่ง reference แทน call)
  ...
  const [currentPrice, setCurrentPrice] = useState(DEFAULT_STOCK.basePrice);
  ...
  // Watchlist — typed เป็น Stock[]
  const [savedWatchlist, setSavedWatchlist] = useState<Stock[]>(() => {
    try {
      const saved = localStorage.getItem('techTick_watchlist_v4');
      if (saved) return JSON.parse(saved) as Stock[];
    } catch {
      // localStorage อาจใช้ไม่ได้ใน environment บางอย่าง
    }
    return [
      { symbol: 'NVDA', name: 'NVIDIA CORP', basePrice: 0, prevClose: 0 },
      { symbol: 'AAPL', name: 'APPLE INC', basePrice: 0, prevClose: 0 },
      { symbol: 'MSFT', name: 'MICROSOFT CORP', basePrice: 0, prevClose: 0 },
      { symbol: 'TSLA', name: 'TESLA INC', basePrice: 0, prevClose: 0 },
    ];
  });
  ...
  const [pinnedPrices, setPinnedPrices] = useState<Record<string, PinnedPrice>>({});
  const [pinErrorMsg, setPinErrorMsg] = useState('');

  // ref สำหรับ guard การ fetch ครั้งแรก — แทน eslint-disable
  ...
  useEffect(() => {
    try {
      localStorage.setItem('techTick_watchlist_v4', JSON.stringify(savedWatchlist));
    } catch { /* ignore */ }
  }, [savedWatchlist]);

  useEffect(() => {
    try {
      localStorage.setItem('techTick_pinned_v4', JSON.stringify(pinnedStocks));
    } catch { /* ignore */ }
  }, [pinnedStocks]);

  // ============================================================
  // Effect — ดึง Exchange Rate
  // ============================================================
  useEffect(() => {
  ...
  // ============================================================
  // Effect — ดึงราคาเริ่มต้น Watchlist (ครั้งเดียว)
  // ใช้ ref guard แทน eslint-disable
  // ============================================================
  useEffect(() => {
    ...
    const fetchInitialPrices = async () => {
      ...
      const updatedWatchlist = await Promise.all(
        savedWatchlist.map(async (stock) => {
      ...
      if (needsUpdate) {
        ...
        if (
          selectedStock.symbol === updatedWatchlist[0]?.symbol &&
          selectedStock.basePrice === 0
        ) {
    ...
    fetchInitialPrices();
  }, [savedWatchlist, selectedStock.symbol, selectedStock.basePrice]);
  ...
  // Memos
  // ============================================================
  const sortedWatchlist = useMemo<Stock[]>(() => {
  ...
  // useMemo แทน let/if ที่คำนวณใหม่ทุก render
  const { strokeColor, statusColorClass, TrendIcon, sign } = useMemo(() => {
    if (priceChange > 0) {
      return {
        strokeColor: '#10b981',
  ...
  // ============================================================
  // Effect — ดึงราคา Pinned stocks
  // เพิ่ม pinnedPrices ใน deps และใช้ functional update เพื่อ
  // หลีกเลี่ยง stale closure
  // ============================================================
  useEffect(() => {
    if (!FINNHUB_API_KEY || pinnedStocks.length === 0) return;

    const fetchPinnedData = async () => {
      const newPrices: Record<string, PinnedPrice> = {};
      ...
      // functional update — ไม่ต้องอ้างอิง pinnedPrices จาก closure
      setPinnedPrices(prev => ({ ...prev, ...newPrices }));
    };

    fetchPinnedData();
    const interval = setInterval(fetchPinnedData, 30_000); // 30s — safe สำหรับ Finnhub free tier
    return () => clearInterval(interval);
  }, [pinnedStocks]);

  // ============================================================
  // Effect — สร้าง Chart labels และ initial chart data
  // ============================================================
  useEffect(() => {
    let points = 50;
    let volatility = 0.02;
  ...
  // Real API: ทุก 15s (safe สำหรับ Finnhub free 60req/min)
  // Simulation: ทุก 1s (ไม่ยิง API)
  // ============================================================
  useEffect(() => {
    const REAL_API_INTERVAL = 15_000;
    const SIM_INTERVAL = 1_000;

    if (FINNHUB_API_KEY) {
      const interval = setInterval(fetchRealData, REAL_API_INTERVAL);
      return () => clearInterval(interval);
    } else {
      // Simulation mode — ไม่ยิง API เลย
  ...
  // ============================================================
  // renderChart — ฟังก์ชัน render SVG กราฟ
  // ============================================================
  const renderChart = () => {
    ...
    return (
      ...
      >
        ...
        {showPrevClose && (
          <g>
            <line
              x1="0" y1={prevCloseY} x2={width} y2={prevCloseY}
              stroke={prevLineColor} strokeDasharray="4 4" strokeWidth="1.5" opacity="0.7"
            />
            <rect x={width - 55} y={prevCloseY - 10} width="55" height="20" rx="4" fill={prevBadgeBg} />
            <text x={width - 27} y={prevCloseY + 3} fill={prevBadgeText} fontSize="10" textAnchor="middle" fontWeight="bold">
              Prev Cls
            </text>
          </g>
        )}
        ...
        <g>
          ...
          {(() => {
            const numLabels = timeframe === '1W' ? 7 : 6;
        ...
        {hoveredIndex !== null && (() => {
          ...
          let hoverSign  = '';
          let HoverIcon  = '−';
          if (pDiff > 0) { hoverColor = '#10b981'; hoverSign = '+'; HoverIcon = '▲'; }
          else if (pDiff < 0) { hoverColor = '#ef4444'; HoverIcon = '▼'; }
                    ...
                    {apiSearchResults.length > 0 ? (
                      apiSearchResults.map(stock => (
                        ...
                        >
                          ...
                          {savedWatchlist.some(s => s.symbol === stock.symbol) && (
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                <div className="flex items-center gap-3">
                  ...
                  >
                    {isWatchlisted
                      ? <><Star className="w-3.5 h-3.5 fill-current" /> Watchlisted</>
                ...
                {['1H', '1D', '1W', '1M', '3M', '1Y'].map(tf => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-xs font-semibold transition-all ${
                      timeframe === tf
                        ? 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow-sm border border-slate-300 dark:border-slate-700'
            ...
            <div className="space-y-2 overflow-y-auto pr-1 pb-4 custom-scrollbar">
              {sortedWatchlist.length === 0 ? (
                ...
                sortedWatchlist.map(stock => {
                  ...
                  const pinData   = isPinned ? pinnedPrices[stock.symbol] : undefined;

                  const sidePrice   = pinData?.c  ?? stock.basePrice ?? 0;
                  const sidePrev    = pinData?.pc ?? stock.prevClose ?? 0;
                  ...
                  else if (sideDiff < 0) { itemColorClass = 'text-red-500 dark:text-red-400'; }
                  ...
                  return (
                    <button
                      ...
                      className={`w-full group flex items-center justify-between p-3 rounded-lg border transition-all text-left relative overflow-hidden ${
                        isActive
                          ? 'bg-white dark:bg-slate-800 border-blue-400 dark:border-blue-500/50 shadow-md'
                          : 'bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800/50'
                      }`}
                    >
                      {isPinned && (
                        <div className="absolute left-0 top-0 bottom-0 w-1 bg-yellow-500/80 rounded-l-lg" />
                      )}
                      ...
                      <div className="flex items-center gap-3 pl-2">
                        <div
                          ...
                          className={`p-1.5 rounded-md transition-colors ${
                            isPinned
                              ? 'text-yellow-500 bg-yellow-100 dark:bg-yellow-500/10'
                              : 'text-slate-400 dark:text-slate-600 hover:text-yellow-500 hover:bg-slate-200 dark:hover:bg-slate-700 opacity-0 group-hover:opacity-100'
                          }`}
                        >
                      ...
                      <div className="text-right">
                        <div className="font-medium text-slate-800 dark:text-slate-200 text-sm">
                          {sidePrice > 0
                            ? formatPrice(sidePrice, currency, exchangeRate)
                            : <Loader2 className="w-3 h-3 animate-spin inline-block text-slate-400" />
                          }
                        </div>
                        {sidePrev > 0 && (
